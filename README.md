...
주소가 표시된다. `/feed.xml`은 법령변화 브리핑 구독 피드이며, 각 상세 화면은
...
현재 공개본 `kr-law-change-20260720-001`에는 행정심판법 시행령 제16조의2의 최근
시행 브리핑 1건만 들어 있다. 문제카드와 형사소송법 제185조ㆍ제294조의5 통합
브리핑은 승인되지 않았으므로 공개본에 포함되지 않는다. 통합 브리핑은 내부 편집
미리보기에서만 확인할 수 있다. 임시 하위 도메인에서는 계속 검색엔진 전체 색인을
차단한다.
