...
운영 배포의 기준 저장소는 `parkkyusang/rulelink-public-web`이다. 기능과 운영 번들은 별도 브랜치와 PR로 올리고, 공개 번들 차단 규칙 테스트·타입 검사·정적 빌드·공개 경로 스모크 검사가 모두 통과한 경우에만 `main`에 병합한다. 일반 `codex/*`와 `main` 커밋은 Vercel Git 자동배포를 만들지 않는다. 실제 운영 공개는 `web/rulelink_public_next/deploy/release.json`을 갱신한 명시적 공개 PR에서만 GitHub Actions의 비밀 배포 훅을 한 번 호출하고, 누적된 최신 `main` 전체와 실제 공개 스냅샷을 확인한다. 브라우저 시각 검수가 필요한 변경만 `preview-*` 브랜치를 사용한다.
...
컨시어지에서 공개 콘텐츠 후보를 만들 수 있지만 공개 웹은 사건정보를 받거나 컨시어지 API를 호출하지 않는다. 공개 체크 상태는 현재 기기에만 저장되고 사건요청 초안으로 복사·전송되지 않는다. 공개 글의 연결은 `/ko/lawyer-workspace` 설명 게이트만 가리키며, 외부 작업공간은 변호사 자격·소속을 확인한 승인 계정과 역할이 분리된 운영 계정만 허용한다.

컨시어지에서 지명한 후보는 사건 본문을 포함하지 않는 내부 편집 작업표로만 전달되고, 비식별화·일반화·활성 DB 재근거화·법률검토·운영 승인을 거쳐야 출판본에 들어온다. 컨시어지 연계는 보조적인 수요 발견 경로이며, 새 법령을 먼저 포착하는 주된 경로는 별도 법령변화 후보 대기열이다.

## 현재 공개본

<!-- RULELINK_PUBLICATION_STATUS:START -->
저장소의 승인 출판본 `kr-knowledge-core-20260723-023`에는 다음 공개 데이터가 연결되어 있다.

- 최근 시행·시행 예정 법령변화: 11건
- 연결 주장: 23개
- 생활법률 지식: 284개
- 주제 허브: 28개
- 법리카드: 278개
...
현재 주제는 행정처분과 행정심판, 소비자·온라인 계약, 가족·상속, 주택임대차와 보증금 반환, 노동·임금, 금전거래·보증, 공유자전거ㆍ공유이동 사고, 일상사고와 손해배상, 이혼·양육·재산분할, 채권추심·강제집행, 부동산 매매·계약금, 형사피해 신고와 피해회복, 헷갈리는 법률개념 비교, 헷갈리는 법률절차와 효과 비교, 어떤 구제절차를 선택해야 하나요?, 놓치면 달라지는 법률기한과 시점 비교, 법률정보와 변호사 이용, 보이스피싱 지급정지·피해환급, 주택임대차 갱신·수선·차임, 민사소송·소액재판, 상가건물 임대차, 누수·층간소음·공용부분 분쟁, 자동차 교통사고·보험 대응, 해고·퇴직·실업 대응, 산업재해·산재보험 대응, 직장 내 괴롭힘·성희롱 대응, 학교폭력 신고·보호·조치·불복, 가정폭력·스토킹 안전대응이다. 각 글은 공식 근거, 핵심 법리, 결론을 가르는 사실, 행동 순서와 보관할 자료를 제공한다. 사건별 분석이 필요한 글은 공개 사건전송 기능 대신 변호사 전용 작업공간의 이용 이유와 자격 확인 절차를 연결한다.
