...
| 시각 작업 세션 | 공개 화면 컴포넌트, 전역 시각 토큰, 공개 정적 애셋과 시각 검수 기록 | 승인 번들 본문, 법리·사실분기·근거 좌표, 출판 스키마와 승인 상태 |
...
3. 시각 작업 세션은 화면 계층만 수정하고, 실제 브라우저 검수가 필요할 때에만 `preview-*` 브랜치를 사용한다.
4. 운영 번들 갱신은 콘텐츠 생산 브랜치와 웹 기능 브랜치를 섞지 않은 별도 PR로 올린다.
5. 하나의 운영 번들 PR이 열려 있는 동안 다른 세션은 `artifacts/publication/current/bundle.json`을 수정하지 않는다.
6. 번들 PR이 병합된 뒤 다음 콘텐츠 세션은 최신 `main`에서 새 브랜치를 시작한다.
7. 공개 웹은 컨시어지 실행기나 내부 API를 호출하지 않는다. 컨시어지 연결은 허용된 별도 주소로 이동하는 링크뿐이다.
...
런타임 스모크는 빌드 서버를 실제로 기동해 승인 번들의 모든 공개 경로와 `publication.json` 스냅샷·건수·비공개 필드 부재를 확인한다. 일반 PR은 GitHub Actions에서 같은 검사를 통과한 경우에만 병합한다. 실제 브라우저 시각 검수가 필요한 변경은 `preview-*` 브랜치의 Vercel 미리보기까지 확인한다. `main` 병합 뒤 운영 Vercel 배포 성공까지 확인해야 인계가 끝난다.

## 9. 출판 뒤 최신성 감시

출판 당시 검증 통과는 영구적인 최신성을 뜻하지 않는다. `.github/workflows/publication-freshness.yml`은 매일 한국시간 오전 7시 17분 현재 승인본을 다시 점검한다.

점검 요약에는 다음 항목을 남긴다.

- 현재 `snapshot_id`와 기준 출처 스냅샷
- 문제카드ㆍ법령변화ㆍ지식 콘텐츠ㆍ허브 수
- 법리카드ㆍ사실분기ㆍ공식 근거 수
- 가장 가까운 재검토 기한과 30일 이내 재검토 대상
- 가장 오래된 공식 근거 확인시각
- 법령변화별 시행일과 현재 날짜의 간격

재검토 기한 경과, 미래 검토일, 시행일과 생애주기 상태 불일치, 불변 출판 이력 불일치는 실패로 처리한다. 공개 조회계층은 매시간 재생성되며 기한이 도래한 문제카드ㆍ법령변화ㆍ지식 콘텐츠와 내용이 비게 된 허브ㆍ주제를 자동 제외한다. 이 자동 제외는 새 법률 내용을 만들거나 기한을 연장하지 않는 실패 차단 장치다. 실패한 공개본을 JSON에서 직접 고치지 않고 내부 Source Maintenance와 콘텐츠 검토 단계에서 새 근거ㆍ새 승인ㆍ새 `snapshot_id`를 생성한다.
