...
test('현행 민법·집합건물법·공동주택관리법 12개 좌표와 효력판을 고정한다', () => {
...
test('원인·수선·공용부분·층간소음·구제수단의 핵심 분리를 고정한다', () => {
  assert.match(rules.get('rule.neighbor-leak-noise.leak-origin-evidence').proposition_ko, /원인.*관리주체.*인과관계/);
  assert.match(rules.get('rule.neighbor-leak-noise.rental-repair-duty').proposition_ko, /사용·수익에 필요한 상태/);
  assert.match(rules.get('rule.neighbor-leak-noise.urgent-repair-reimbursement').proposition_ko, /보존에 필요한 비용/);
...
test('임대차·매매하자·일상손해·소액사건 정본으로 실제 외부 연결을 닫는다', () => {
  const required = new Map([
    ['content.neighbor-leak-noise-rental-repair-duty',['content.housing-lease-living-repair-duty']],
    ['content.neighbor-leak-noise-urgent-repair-reimbursement',['content.housing-lease-living-repair-reimbursement']],
    ['content.neighbor-leak-noise-leak-origin-evidence',['content.accident-evidence-and-tort-limitation','content.hidden-defect-after-purchase']],
