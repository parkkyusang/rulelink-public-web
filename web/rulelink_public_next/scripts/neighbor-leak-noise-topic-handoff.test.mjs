...
test('현행 민법·민사소송법·집합건물법·공동주택관리법 12개 좌표와 효력판을 고정한다', () => {
  const expected = new Map([
    ...
    ['civil_procedure_ko_0376','snapshot:b20085997d0047c270a71a3f352c50b9'],
    ...
    ['aggregate_buildings_ko_0016','snapshot:5efc0ec9301b5c8de1beba0aacdb5c38'],
    ['aggregate_buildings_ko_0025','snapshot:0673814d538b9d1e1ed90911a4759e6c'],
  ...
  for (const source of topic.sources) {
    ...
    assert.match(source.source_snapshot_id, /^snapshot:[0-9a-f]{32}$/u, `${source.source_id}: 32자리 스냅샷이 아닙니다.`);
    ...
    assert.equal(source.last_verified_at, '2026-07-23T00:50:00+00:00');
  }
});

test('근거 검증시각과 콘텐츠 검토시각은 실제 실행시각보다 미래일 수 없다', () => {
  const now = Date.now();
  const timestamps = [
    ...topic.sources.map(source => [source.coordinate_id, 'last_verified_at', source.last_verified_at]),
    ...topic.content_entries.map(entry => [entry.content_id, 'reviewed_at', entry.reviewed_at]),
  ];
  assert.equal(timestamps.length, 22);
  for (const [id, field, value] of timestamps) {
    const parsed = Date.parse(value);
    assert.ok(Number.isFinite(parsed), `${id}: ${field}가 유효한 ISO 시각이 아닙니다.`);
    assert.ok(parsed <= now, `${id}: ${field}가 미래 시각입니다: ${value}`);
    assert.equal(value, '2026-07-23T00:50:00+00:00', `${id}: 이번 실제 검증·검토시각과 다릅니다.`);
  }
});
...
test("#155가 편집한 본문·주의문·검색문구를 그대로 보존한다", () => {
  const editorialCopy = topic.content_entries.map(({
    content_id,
    body_sections,
    caution_ko,
    search_intents_ko,
  }) => ({content_id, body_sections, caution_ko, search_intents_ko}));
  const digest = createHash('sha256').update(JSON.stringify(editorialCopy)).digest('hex');
  assert.equal(digest, '09716efb7e47fa0faf93d5f5e40dd2d4985754a018ef0c6375c4bb85d34e94b9');
});
