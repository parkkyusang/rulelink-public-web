...
test('현재 공개 콘텐츠 생산 대기열은 역할·중복·의존성 계약을 만족한다', () => {
  ...
  assert.equal(queue.items.filter(item => item.status === 'ready_for_integration').length, 10);
  assert.equal(queue.items.filter(item => item.status === 'needs_rework').length, 7);
...
test('현재 정본과의 의미 중복은 content_id 대상으로 기록할 수 있다', () => {
  const value = clone(queue);
  const item = value.items.find(candidate => candidate.pr_number === 127);
  item.overlap_decisions = [{
    target_content_id: 'content.labor-commission-rereview-vs-lawsuit-deadline',
    relationship: 'supersedes',
    rationale_ko: '현재 정본의 중복 질문을 새 주제에서 대체합니다.',
  }];
  assert.deepEqual(validateProductionQueue(value), []);

  const invalid = clone(value);
  invalid.items.find(candidate => candidate.pr_number === 127).overlap_decisions[0].target_pr = 107;
