...
test('법률용어 해설은 마우스 전용이 아니며 클릭·탭·키보드에서 뜻과 독립 페이지를 함께 제공한다', async () => {
  ...
  assert.match(component, /onClick=\{event =>/);
  ...
  assert.match(component, /href=\{`\/ko\/concepts\/\$\{activePart\.concept\.slug\}`\}/);
  assert.match(component, /role="group"/);
  assert.match(component, /<FloatingPortal>/);
  assert.match(component, /<FloatingFocusManager[^>]*initialFocus=\{refs\.floating\}[^>]*modal=\{false\}/);
