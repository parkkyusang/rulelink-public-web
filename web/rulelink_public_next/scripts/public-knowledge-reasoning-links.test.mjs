...
test('연결 법리 라벨은 비조작 텍스트이고 실제 법리만 순서대로 초점 가능한 앵커다', async () => {
  ...
  const markup = renderToStaticMarkup(createElement(
    ...
    createElement(ScenarioRuleLinks, {
      classes: styles,
      rules,
      scenarioId: 'scenario.legal-service-boundaries.revenue-structure',
    }),
  ));

  const label = markup.match(/<span([^>]*)>연결 법리<\/span>/u);
  assert(label, '연결 법리 라벨이 렌더링되어야 합니다.');
  assert.match(label[1], /id="([^"]+)"/u);
  ...
  const labelId = label[1].match(/id="([^"]+)"/u)?.[1];
  assert(labelId);
  assert.match(markup, new RegExp(`<nav[^>]*aria-labelledby="${labelId}"[^>]*>`, 'u'));
