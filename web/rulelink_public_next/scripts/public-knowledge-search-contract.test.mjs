...
const [bundle, projectionSource, rankingSource, siteSearchSource, siteSearchIndexSource, siteSearchPublicationSource, searchPageSource, legacySearchRouteSource, searchV2RouteSource, searchRouteSource, knowledgeExplorerSource] = await Promise.all([
  ...
  readFile(path.join(root, 'app', 'search-index.v3.json', 'route.ts'), 'utf8'),
...
test('통합검색과 지식 보관함은 검색 투영과 연결 근거 표지를 사용한다', () => {
  ...
  assert.match(searchPageSource, /indexHref="\/search-index\.v3\.json"/);
  assert.match(siteSearchSource, /decodeSiteSearchIndex/);
  assert.match(siteSearchIndexSource, /rulelink_public_search_index_v3/);
...
test('통합검색은 전체 검색 투영을 유지하고 화면 카드만 점진적으로 표시한다', () => {
  ...
  assert.match(searchV2RouteSource, /encodeSiteSearchIndexV2/);
