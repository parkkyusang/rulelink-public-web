...
test('해고·퇴직·실업 10개 생활질문의 근거·법리·사실분기를 닫는다', () => {
  ...
  for (const coordinateId of sources.keys()) {
    assert.ok(referencedCoordinates.has(coordinateId), `역참조되지 않은 근거: ${coordinateId}`);
  }
});
...
test('활성 DB에서 확인한 현행 조문 13개의 공식 좌표와 스냅샷을 고정한다', () => {
  const expected = new Map([
    ...
    ['labor_standards_act_ko_0025', 'snapshot:cb86bd89360f914a3400296869387675'],
  ...
  for (const source of topic.sources) {
    ...
    const lawLevelUrl = new URL(
      `https://www.law.go.kr/${['법령', source.law_name_ko.trim()].map(encodeURIComponent).join('/')}`,
    ).href;
    const expectedOfficialUrl = new URL(
      `https://www.law.go.kr/${['법령', source.law_name_ko.trim(), source.article_no.trim()].map(encodeURIComponent).join('/')}`,
    ).href;
    assert.notEqual(source.official_url, lawLevelUrl, `${source.coordinate_id}: 법률 단위 URL은 허용하지 않습니다.`);
    assert.equal(source.official_url, expectedOfficialUrl, `${source.coordinate_id}: 법률명·조문번호 고정 주소가 아닙니다.`);
    assert.ok(!source.official_url.includes('lawView.do'));
    assert.ok(['근로기준법', '근로자퇴직급여 보장법', '고용보험법'].includes(source.law_name_ko));
    assert.equal(source.last_verified_at, '2026-07-23T00:00:00+00:00');
  }
});
...
test('기존 임금 주제의 중복을 피하고 퇴직 전후의 빈 경로를 연결한다', () => {
  ...
  for (const existingTitle of [
    ...
    '노동위원회 재심 10일과 소송 15일은 언제 계산하나요?',
  ...
  for (const id of [
    ...
    'content.labor-commission-rereview-vs-lawsuit-deadline',
  ...
  assert.match(rules.get('rule.employment-exit.managerial-reemployment').norm.conditions_ko, /3년 이내.*같은 업무.*원하는/);
  ...
  assert.ok(!entries.has('content.employment-exit-remedy-appeal-deadlines'));
  assert.ok(!rules.has('rule.employment-exit.remedy-appeal'));
  assert.ok(!scenarios.has('scenario.employment-exit.after-decision'));
  assert.deepEqual(
    entries.get('content.employment-exit-managerial-reemployment').related_content_ids,
    [
      'content.employment-exit-managerial-dismissal',
      'content.employment-exit-monetary-remedy',
      'content.labor-commission-rereview-vs-lawsuit-deadline',
    ],
  );
});
