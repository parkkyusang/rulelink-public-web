...
function checkStableKnowledgeSource(source, name, errors) {
  ...
  const lawPathNames = new Set([
    source.law_name_ko.trim(),
    source.law_name_ko.replace(/\s+/gu, ''),
  ]);
  const expectedUrls = new Set([...lawPathNames].map(lawPathName => (
    new URL(`https://www.law.go.kr/${['법령', lawPathName, source.article_no.trim()].map(encodeURIComponent).join('/')}`).href
  )));
  try {
    if (!expectedUrls.has(new URL(source.official_url).href)) {
