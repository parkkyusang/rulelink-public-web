...
test('상가건물 임대차 11개 생활질문의 근거·법리·사실분기를 닫는다', () => {
  ...
  for (const entry of entries.values()) {
    ...
    assert.ok(Date.parse(entry.reviewed_at) <= Date.now(), `${entry.content_id}: 검토일이 현재보다 미래입니다.`);
...
test('활성 DB에서 확인한 현행 조문 16개의 공식 좌표와 스냅샷을 고정한다', () => {
  ...
  for (const source of topic.sources) {
    ...
    assert.equal(source.official_url, expectedOfficialUrl);
    assert.ok(!source.official_url.includes('lawView.do'));
    assert.equal(source.law_name_ko, '상가건물 임대차보호법');
    assert.ok(Date.parse(source.last_verified_at) <= Date.now(), `${source.coordinate_id}: 근거 검증시각이 현재보다 미래입니다.`);
  }
});
