...
test('10개 글의 타입 관계와 기존 related_content_ids 투영을 정확히 고정한다', () => {
  ...
  for (const [contentId, expected] of EXPECTED_RELATED) {
    ...
    assert.equal(new Set(projectedContentIds).size, projectedContentIds.length, `${contentId}: 중복 대상`);
    ...
    for (const edge of entry.related_edges) {
      ...
      directedPairs.add(`${contentId}->${edge.target_id}`);
    }
  }

  for (const pair of directedPairs) {
    const [from, to] = pair.split('->');
    assert.ok(!directedPairs.has(`${to}->${from}`), `의미 없는 양방향 관계: ${from} <-> ${to}`);
  }
});
...
function assertAcyclicWithinTopic() {
  const visiting = new Set();
  const visited = new Set();

  function visit(contentId) {
    if (visiting.has(contentId)) assert.fail(`동일 주제 순환 관계: ${contentId}`);
    if (visited.has(contentId)) return;
    visiting.add(contentId);
    for (const targetId of entries.get(contentId)?.related_content_ids ?? []) {
      if (entries.has(targetId)) visit(targetId);
    }
    visiting.delete(contentId);
    visited.add(contentId);
  }

  for (const contentId of entries.keys()) visit(contentId);
}
