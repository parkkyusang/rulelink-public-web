...
test('공식 URL 22개는 실제 GET 200으로 응답한다', {timeout: 30_000}, async () => {
  const responses = await Promise.all(topic.sources.map(async source => {
    const response = await fetch(source.official_url, {redirect: 'follow'});
    return [source.source_id, response.status];
  }));
  assert.deepEqual(responses, topic.sources.map(source => [source.source_id, 200]));
});
