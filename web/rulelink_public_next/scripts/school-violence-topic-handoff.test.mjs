...
const standardContentTypes = new Set([
  'law_change',
  'doctrine_explainer',
  'fact_branch',
  'precedent_doctrine',
...
test('학교폭력 10개 생활질문의 근거·법리·사실분기를 닫는다', () => {
  ...
  for (const entry of entries.values()) {
    ...
    assert.ok(standardContentTypes.has(entry.content_type), `${entry.content_id}: 비표준 콘텐츠 유형 ${entry.content_type}`);
...
test('활성 DB에서 확인한 학교폭력예방법 현행 조문 12개의 좌표와 스냅샷을 고정한다', () => {
  const expected = new Map([
    ...
    ['law_009620_ko_0017', 'snapshot:e4072a5574300cdfb17e013673dc3ea3'],
    ...
    ['law_009620_ko_0020', 'snapshot:217c46e33812b9c1891db093bf615132'],
  ...
  for (const source of topic.sources) {
    ...
    assert.match(source.source_snapshot_id, /^snapshot:[0-9a-f]{32}$/, `${source.source_id}: 스냅샷 형식 오류`);
    ...
    assert.ok(source.official_url.endsWith(`/${encodeURIComponent(source.article_no)}`), `${source.source_id}: 조문 직접 주소가 아닙니다.`);
    assert.ok(Date.parse(source.last_verified_at) <= Date.now(), `${source.source_id}: 미래 검증시각입니다.`);
    officialUrls.add(source.official_url);
  }
  assert.equal(officialUrls.size, topic.sources.length, '조문별 공식 주소가 고유하지 않습니다.');
});

test('12개 공식 근거는 법리·분기·콘텐츠 중 하나 이상에서 모두 역참조된다', () => {
  ...
  assert.deepEqual([...sources.keys()].sort(), [...referenced].sort());
  for (const entry of entries.values()) {
    assert.ok(Date.parse(entry.reviewed_at) <= Date.now(), `${entry.content_id}: 미래 검토시각입니다.`);
  }
});
...
test('신고·자체해결·피해보호·가해조치·불복의 핵심 법리를 고정한다', () => {
  ...
  const reportRule = rules.get('rule.school-violence.report');
  const reportScenario = scenarios.get('scenario.school-violence.report');
  const reportEntry = entries.get('content.school-violence-report');
  assert.match(reportRule.proposition_ko, /실제 학교폭력 사실.*즉시 신고.*예비·음모.*고발할 수/);
  assert.match(reportRule.norm.conditions_ko, /이미 발생한 실제 사실.*예비·음모 단계/);
  assert.match(reportRule.norm.legal_effect_ko, /실제 사실.*즉시 신고해야.*예비·음모.*고발할 수.*교원.*보고.*학부모.*불이익/);
  assert.match(reportScenario.question_ko, /이미 발생한 학교폭력 사실.*예비·음모 단계/);
  assert.match(reportScenario.when_true_ko, /이미 발생한 사실.*즉시 신고/);
  assert.match(reportScenario.when_false_ko, /예비·음모 단계.*고발할 수.*교원.*보고.*학부모/);
  assert.match(reportEntry.one_line_answer_ko, /실제 사실.*즉시 신고.*예비·음모.*고발할 수/);
  assert.match(reportEntry.key_points_ko.join(' '), /발생한 학교폭력 사실.*즉시 신고.*예비·음모.*고발할 수.*교원.*보고·통지/);
  assert.match(reportEntry.action_steps_ko.join(' '), /실제 사실.*즉시 신고.*예비·음모.*필요에 따라.*고발할 수/);
  assert.match(reportEntry.caution_ko, /즉시 신고 의무.*예비·음모.*고발 가능.*같은 의무로 보아서는 안/);
  assert.match(reportEntry.body_sections.flatMap(section => section.paragraphs_ko).join(' '), /실제 사실.*즉시 신고.*반면.*예비·음모.*고발할 수.*교원.*보고.*학부모/);
  const collectStrings = value => typeof value === 'string'
    ...
    : Array.isArray(value)
      ...
      : value && typeof value === 'object'
        ...
        : [];
  const preparationStatements = collectStrings([reportRule, reportScenario, reportEntry]);
  for (const statement of preparationStatements) {
    assert.doesNotMatch(statement, /예비·음모[^.]{0,45}즉시 신고(?:해야| 의무)/, `예비·음모를 즉시 신고 의무로 오안내합니다: ${statement}`);
  }
...
test('새 식별자는 미출판 때 충돌하지 않고 출판 뒤에는 현재 정본과 정확히 일치한다', () => {
  const newIds = [
