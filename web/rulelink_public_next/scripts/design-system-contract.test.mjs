...
test('조문 읽기 카드도 공통 반응형 원칙과 무횡스크롤 계약을 지킨다', async () => {
  const authorityCss = await readFile(path.join(
    root,
    'src',
    'components',
    'authority-reading-section.module.css',
  ), 'utf8');
  assert.match(authorityCss, /grid-template-columns: repeat\(2, minmax\(0, 1fr\)\)/);
  assert.match(authorityCss, /@media \(max-width: 767px\)[\s\S]*grid-template-columns: minmax\(0, 1fr\)/);
  assert.match(authorityCss, /\.card\s*\{[^}]*min-width: 0/s);
  assert.match(authorityCss, /overflow-wrap: anywhere/);
  assert.doesNotMatch(authorityCss, /overflow-x:\s*(?:auto|scroll)|line-clamp|text-overflow:\s*ellipsis/);
});
