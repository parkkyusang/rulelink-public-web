...
test('검색 관련어는 근거 있는 완전 동의어·약어·표기변형만 같은 본문 해설을 쓴다', () => {
  const terms = inlineTermsForConcept({
    ...
    term_relations: [
      {term_ko: '법정상속인', relation: 'narrower', target_concept_id: 'concept.legal-heir', source_coordinate_ids: ['source.1000']},
      {term_ko: '공동상속인', relation: 'related', target_concept_id: 'concept.co-heir', source_coordinate_ids: ['source.1003']},
...
test('용어 관계 계약은 관계 미분류·근거 누락·본문 자동 해설 중복을 차단한다', () => {
  ...
  assert.throws(
    () => validateConceptTermRelations([{...valid, term_relations: []}], sources),
    /관계 분류가 없습니다/,
  );
  assert.throws(
    () => validateConceptTermRelations([
...
test('법률용어 팝오버는 카드 overflow와 무관한 충돌 회피 포털에 표시된다', async () => {
  ...
  assert.match(component, /data-layout=\{isMobile \? 'sheet' : 'anchored'\}/);
