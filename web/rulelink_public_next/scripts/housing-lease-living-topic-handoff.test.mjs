...
test('주택임대차 유지단계 10개 질문의 근거·법리·사실분기를 닫는다', () => {
  ...
  for (const id of sources.keys()) assert.ok(referencedSources.has(id), id + " : unreferenced source");
...
test('현행 민법·주택임대차법·신고법 13개 좌표와 스냅샷을 고정한다', () => {
  const expected = new Map([
    ...
    ['law_012790_ko_0004_03','snapshot:fdcc934869d52e4effcd5087ff86dbbf'],
  ]);
...
test('13개 근거와 10개 콘텐츠의 실제 감사시각 및 만료 순서를 고정한다', () => {
...
test('감사시각 외 법률·근거·관계·편집값과 만료시각은 불변이다', () => {
  assert.equal(
    ...
    '397ae62db55de264ade845d9bf269cdd20a97702bdcdc9e5061039a95c69c917',
  );
});
...
test('수선·2기연체·갱신·5퍼센트·30일의 오해방지 기준을 고정한다', () => {
  ...
  assert.match(rules.get('rule.housing-lease-living.rent-arrears').proposition_ko, /2기의 차임액.*계약을 해지/);
  assert.match(rules.get('rule.housing-lease-living.rent-arrears').norm.legal_effect_ko, /자동 종료를 뜻하지/);
